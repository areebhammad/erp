...
function ResetPasswordPage() {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            try {
                await resetPasswordApi({ token: search.token, new_password: value.password });
                addNotification({
                ...
                if (resStatus === 400 || resStatus === 410 || resStatus === 401) {
                    ...
                    setSubmitError(e?.response?.data?.message || 'Failed to reset password');
                }
            }
        },
    });
    ...
    return (
        <div className="w-full">
            <h3 className="text-xl font-semibold mb-6 text-text">Create new password</h3>
            ...
            >
                    ...
                    {(field) => {
                        ...
                        return (
                            <div className="space-y-1">
                                <label className="text-sm font-medium text-text-subtle" htmlFor={field.name}>New Password</label>
                ...
                <form.Field name="confirm_password">
                    {(field) => (
                        <div className="space-y-1">
                            <label className="text-sm font-medium text-text-subtle" htmlFor={field.name}>Confirm New Password</label>
                            <div className="relative">
                                <input
                                    id={field.name}
                                    name={field.name}
