...
const handlers = [
    http.get(`${env.VITE_API_URL}/api/v1/auth/refresh`, () => {
        ...
        return HttpResponse.json({ code: 'unauthorized', message: 'Token expired' }, { status: 401 });
    }),
    ...
    http.post(`${env.VITE_API_URL}/test-post`, ({ request }) => {
        return HttpResponse.json({ success: true, method: request.method, csrf: request.headers.get('X-CSRF-Token') }, { status: 200 });
    }),
];
...
describe('API Client', () => {
    ...
    beforeEach(() => {
        ...
        mockCookie = '';
        vi.clearAllMocks();
        useAuthStore.setState({ isAuthenticated: true, user: { id: 'test', email: 'test@example.com' }, sessionId: 's1' });
